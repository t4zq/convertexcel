...
const SEVERITY: Record<DiagnosticSeverity, { glyph: string; color: string }> = {
...
export function InputDiagnosticsPanel({
  ...
  const delimiterLabel = t.diagnostics.delimiters[diagnostics.format.delimiter]
  ...
  return (
    <div className="overflow-hidden rounded-md border bg-muted/40 font-mono text-xs">
      ...
      <div className="grid gap-2 border-b px-3 py-2 text-muted-foreground sm:grid-cols-4">
        <span>{t.diagnostics.format}: <strong className="text-foreground">{delimiterLabel}</strong></span>
        <span>{t.diagnostics.rows}: <strong className="text-foreground">{diagnostics.rowCount}</strong></span>
        <span>{t.diagnostics.columns}: <strong className="text-foreground">{diagnostics.maxCols}</strong></span>
        <span>{t.diagnostics.normalized}: <strong className="text-foreground">{diagnostics.format.changedCells}</strong> {t.diagnostics.cells}</span>
      </div>
      {diagnostics.normalizedRows.length > 0 && (
        <div className="border-b px-3 py-2">
          <div className="mb-1 text-muted-foreground">{t.diagnostics.normalizedPreview}</div>
                  ...
                  <tr key={rowIndex} className="border-b last:border-b-0">
                    ...
                    {Array.from({ length: diagnostics.maxCols }, (_, columnIndex) => (
                      <td key={columnIndex} className="max-w-40 truncate border-r px-2 py-1 last:border-r-0">
                        {row[columnIndex] || <span className="text-muted-foreground/60">{t.diagnostics.emptyCell}</span>}
                      </td>
                    ))}
                  </tr>
                ))}
          ...
          {diagnostics.normalizedRows.length > 4 && (
            <div className="mt-1 text-muted-foreground">
              {t.diagnostics.moreRows(diagnostics.normalizedRows.length - 4)}
            </div>
          )}
        </div>
      )}
          ...
          {problems.map((p, i) => {
            ...
            return (
              <li key={`${p.code}-${p.line ?? i}-${p.column ?? 0}`} className="flex items-baseline gap-2 px-3 py-1.5">
                ...
                <span className={`${s.color} shrink-0 font-semibold`}>{t.diagnostics.severity[p.severity]}:</span>
                ...
                <span className="ml-auto shrink-0 whitespace-nowrap text-muted-foreground/80">
                  ...
                  {p.column != null && <span className="text-info/80">{t.diagnostics.column(p.column)} </span>}
                  {p.code}
                </span>
              </li>
            )
