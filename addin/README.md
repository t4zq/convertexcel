...
- 配布する manifest は `addin/manifest.prod.xml`（`https://convertexcel.net/addin/addin.html` を指す）です。利用者はこの XML を Excel に sideload します。開発用の `addin/manifest.xml`（localhost を指す）と混同しないでください。
